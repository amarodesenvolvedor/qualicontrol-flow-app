...
export function AppSidebar({ isMobileMenuOpen, onMenuClose }: AppSidebarProps) {
  ...
  return <Sidebar className={collapsed && !isMobile ? "w-16" : "w-64"} collapsible="icon">
      <div className="flex items-center justify-between p-4">
        {(!collapsed || isMobile) && (
          <span className="text-xl font-bold text-sidebar-foreground">IntegraQMS - SEW</span>
        )}
        ...
        {isMobile ? (
          <Button 
            ...
            size="icon" 
            onClick={onMenuClose} 
            className="self-end text-sidebar-foreground"
          >
            <X className="h-5 w-5" />
          </Button>
        ) : (

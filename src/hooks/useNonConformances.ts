...
export const useNonConformances = () => {
  ...
  const createNonConformance = useMutation({
    mutationFn: async (nonConformance: NonConformanceInput) => {
      ...
      // The 'code' field is automatically generated by the database trigger
      // Extract only the fields we need, excluding code which is generated by the DB
      const { 
        title,
        description,
        location,
      ...
      // Fix: Use .select() instead of .insert().select() which expects a 'code' field
      const { data, error } = await supabase
        .from("non_conformances")
